...
"jump" <-
function(data=NULL,K=10,y=NULL,plotjumps=T,fits=NULL,B=0,dist=NULL,trace=F){
  if (!is.null(data)){
    # Compute the kmeans fit to the data
  if (is.null(fits))
  fits <- kmeans.rndstart(data,K)
  if (is.null(y))
    y <- dim(data)[2]/2
  n <- nrow(data)
  p <- ncol(data)
  ...
  jump.results <- compute.jump(dist,y,plotjumps,trace)
  ...
  for (b in 1:B){
    ...
    bootfits <- kmeans.rndstart(bootdata,K)
